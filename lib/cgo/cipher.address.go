...
//export SKY_cipher_MustAddressFromSecKey
func SKY_cipher_MustAddressFromSecKey(_secKey *C.cipher__SecKey, _arg1 *C.cipher__Address) (____error_code uint32) {
	____error_code = 0
	secKey := *(*cipher.SecKey)(unsafe.Pointer(_secKey))
	__arg1 := cipher.MustAddressFromSecKey(secKey)
	*_arg1 = *(*C.cipher__Address)(unsafe.Pointer(&__arg1))
	return
}
...
func SKY_cipher_MustAddressFromBytes(_b []byte, _arg1 *C.cipher__Address) (____error_code uint32) {
	____error_code = 0
	b := *(*[]byte)(unsafe.Pointer(&_b))
	__arg1 := cipher.MustAddressFromBytes(b)
	*_arg1 = *(*C.cipher__Address)(unsafe.Pointer(&__arg1))
	return
}
